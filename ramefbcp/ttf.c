/*
  ttf.c: TTF rendering functions; based on SDL2_ttf but simplified.
...
static void TTF_SetError(const char *msg)
{
    if (msg == NULL)
        ttf_errbuf[0] = 0;
    else
        strncpy(ttf_errbuf, msg, TTF_ERRBUFSIZE);
    ttf_errbuf[TTF_ERRBUFSIZE - 1] = 0;
}
...
/***** Local replacements for SDL_ helper functions *****/
...
static long long s_rw_fseek(SDL_RWops * context, Sint64 offset, int whence)
{
...
#define SDL_RWseek(ctx, offset, whence) s_rw_fseek(ctx, offset, whence)
#define SDL_RWtell(ctx) ftell(ctx)
#define SDL_RWread(ctx, ptr, size, n) fread(ptr, size, n, ctx)
#define SDL_RWwrite(ctx, ptr, size, n) fwrite(ptr, size, n, ctx)
#define SDL_RWclose(ctx) fclose(ctx)

static long long s_rw_filesize(SDL_RWops * context)
{
    long long pos, size;
    //printf("TTF RW s_rw_filesize ctx:%08x\n", (unsigned int)context);
    pos = SDL_RWseek(context, 0, SEEK_CUR);
    if (pos < 0) {
        //printf("-result: -1\n");
        return -1;
    }
    size = SDL_RWseek(context, 0, SEEK_END);
    SDL_RWseek(context, pos, SEEK_SET);
    //printf("-result %lld\n", size);
    return size;
}

#define SDL_RWsize(ctx) s_rw_filesize(ctx)
...
#define TTF_CHECKPOINTER_NO_ERRVAL(p)                 \
    if ( !TTF_initialized ) {                   \
...
)
{
    ...
    //printf("RWread %08x offs:%u %08x count:%u F*:%08x\n",
    //       (unsigned int)stream, (unsigned int)offset,
    //       (unsigned int)buffer, (unsigned int)count, (unsigned int)src);

    SDL_RWseek( src, (int)offset, SEEK_SET ); // was RW_SEEK_SET
    if ( count == 0 ) {
        return 0;
    }
...
TTF_Font* TTF_OpenFontIndex( const char *file, int ptsize, long index )
{
    ...
    if ( rw == NULL ) {
        ...
        TTF_SetError("TTF cannot open font file");
        return NULL;
    }
...
int TTF_GlyphIsProvided(const TTF_Font *font, unsigned short ch)
{
  return(FT_Get_Char_Index(font->face, ch));
}

int TTF_GlyphMetrics(TTF_Font *font, unsigned short ch,
...
#define SDL_OutOfMemory() do { TTF_SetError("TTF out of memory"); } while (0)
...
TTF_Surface *TTF_RenderText_Solid(TTF_Font *font,
                                  const char *text)
{
    TTF_Surface *surface = NULL;
    Uint8 *utf8;
    ...
    if ( utf8 ) {
        LATIN1_to_UTF8(text, utf8);
        surface = TTF_RenderUTF8_Solid(font, (char *)utf8);
        SDL_stack_free(utf8);
    } else {
        SDL_OutOfMemory();
    }
...
TTF_Surface * TTF_CreateSurface(int width, int height)
{
    TTF_Surface *res = (TTF_Surface *)malloc(sizeof(TTF_Surface));
    if (res == 0)
    {
        TTF_SetError("TTF_CreateSurface failed");
        return NULL;
    }
    res->w = width;
    res->h = height;
    res->pitch = (unsigned short)width;
    res->pixels = malloc(res->pitch * res->h);
    if (res->pixels == 0)
    {
        free(res);
        TTF_SetError("TTF_CreateSurface pixel alloc failed");
        return NULL;
    }
    memset(res->pixels, 0, res->pitch * res->h);
    return res;
}

void TTF_FreeSurface(TTF_Surface *surface)
{
    if (surface == NULL)
        return;
    free(surface->pixels);
...
void TTF_ClearSurface(TTF_Surface *surface)
{
...
TTF_Surface *TTF_RenderUTF8_Solid(TTF_Font *font,
                                  const char *text)
{
    SDL_bool first;
    int xstart;
    ...
    textbuf = TTF_CreateSurface(width, height);
    if ( textbuf == NULL ) {
        return NULL;
    }
...
TTF_Surface *TTF_RenderGlyph_Solid(TTF_Font *font, unsigned short ch)
{
    Uint16 ucs2[2] = { ch, 0 };
    Uint8 utf8[4];

    UCS2_to_UTF8(ucs2, utf8);
    return TTF_RenderUTF8_Solid(font, (char *)utf8);
}

TTF_Surface *TTF_RenderText_Shaded(TTF_Font *font, const char *text)
{
    TTF_Surface *surface = NULL;
    Uint8 *utf8;
    ...
    if ( utf8 ) {
        LATIN1_to_UTF8(text, utf8);
        surface = TTF_RenderUTF8_Shaded(font, (char *)utf8);
        SDL_stack_free(utf8);
    } else {
        SDL_OutOfMemory();
    }
...
*/
TTF_Surface *TTF_RenderUTF8_Shaded(TTF_Font *font, const char *text)
{
    SDL_bool first;
    int xstart;
    int width;
...
void TTF_RenderUTF8_Shaded_Surface(TTF_Surface *dest,
                                   TTF_Font *font, const char *text)
{
    ...
    TTF_CHECKPOINTER_NO_ERRVAL(text);
...
TTF_Surface* TTF_RenderGlyph_Shaded(TTF_Font *font, unsigned short ch)
{
    Uint16 ucs2[2] = { ch, 0 };
    Uint8 utf8[4];

    UCS2_to_UTF8(ucs2, utf8);
    return TTF_RenderUTF8_Shaded(font, (char *)utf8);
}
